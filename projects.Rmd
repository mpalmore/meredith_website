...
Maternal body mass index (BMI) is a known risk factor of child health outcomes, such as childhood obesity. Studies have found single-site epigenetic loci that are associated with maternal pre-pregnancy BMI. I aim to summarize the impact of maternal BMI on the fetal side of the placental epigenome in an epigenetic risk score.
...
* The M-value matrix in this example is named M_regress absmeans are the average of the absolute value of effect sizes.
...
* As Ellen has written it here, the variable name should be in quotes
...
Here is a link to the study from which I derived summary statistics:
<https://pubmed.ncbi.nlm.nih.gov/32071425/>

Here is a link to the study where I derived my GEO dataset:
<http://dx.doi.org/10.1080/15592294.2016.1195534>


Both the summary statistics and methylation data are measured from placental tissue on the 450K methylation array. 

Summary statistics were derived from linear models adjusted for surrogate variables as proxy for placenta cellular heterogeneity, SNP-based ancestry principal components, methylation principal components, maternal age, race and ethnicity, offspring sex,and sequencing batches. 

Load in the data. There are multiple files for the summary statistics becaues they were converted from a pdf file.
...
# One logFC value missing, remove
...
There are two possible ways that I will deal with the probes that do not transfer:
Either... 

1. drop them from the analysis

2. find the nearest probes that we have measured and retain those for the score
...
Now that we have imputed the missing probes with neary ones and updated them in the Methylation matrix and in the top_11 object (summary statistics object), we can proceed with our computation of the score. 
...
Since the outcome is not binary, the baseline we are comparing to is individuals with 'normal' BMI. So when comparing methylation to the the median value of the "unexposed", the unexposed is individuals with normal BMI. 
...
score_ts_byM_p <- function(variable, pthresh) {
  ...
  print(top_11)
  fwrite(top_11, file.path(paste0("cpg_list_in_score_", pthresh, "_", variable, ".csv")))
  ...
  fwrite(top_count_t_11, file.path(paste0("cpg_count_in_score_", pthresh, "_", variable, ".csv")))
  ...
  #Ellen says:
  # for loop to add in median M value for unexposed for each CpG and time period
  # don't need this level of row-by-row detail as they should all be the same tf, but didn't want to change something that worked from my earlier function

  # Meredith says:
  # For me, I don't even have time periods so I'm modifying:
...
Now that we have computed the scores, we can make functions for the results. 
...
gress_graph <- function(score){
  ...
  print(aic(mod))
  # print(NagelkerkeR2(mod))
  #indv_scores <- as.vector(indv_scores)
  #print(auc(bmicat ~ indv_scores, data = score))
   ...
   ggplot(score, aes(x = bmicat, y = indv_scores)) +
     ...
     #labs(title = paste0(colnames(score)[3],";"," Mean change in score = ", round(mean_change[2],2), " C.I. (",round(conf[2,1],2), ",", round(conf[2,2],2),")"),
#          )
...
* Did not account for global differences in methylation, just single site changes. The normalization steps in the publicly available studies may have diluted global changes.
